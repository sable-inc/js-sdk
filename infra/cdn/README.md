...
| --- | --- | --- | --- |
| `https://sdk.withsable.com/v0.1.0/sable.js` | exact version | 1 year, immutable | **Recommended for production.** Upgrades are opt-in, bytes never change under a URL. |
...
- **Layout staged on disk before deploy:**
  ...
  ├── v0.1.0/sable.js          # the built IIFE for the tag being released
...
- Previous versions are preserved because Cloudflare Pages does incremental
  deploys — the `v0.0.2/` directory stays in place when `v0.1.0/` is added.
...
```sh
curl -sI "https://sdk.withsable.com/v0.1.0/sable.js?t=$RANDOM" | head
